...
function gas_station(strArr)
{
const N=parseInt(strArr[0])
console.log(N);

const gasStation=strArr.slice(1).map(station=>{
    const [gallon,cost]=station.split(':').map(Number);
   return {gallon,cost};
});

for(let start=0; start<N; i++)
{
    let remaingallon=0;
    for(let i=0; i<N; i++)
    {
        let currentstation=(start+i) %N;
        remaingallon+=gasStation[currentstation].gallon-gasStation[currentstation].cost;
...
console.log(gas_station(["4","3:1","2:2","1:2","0:1"]))
